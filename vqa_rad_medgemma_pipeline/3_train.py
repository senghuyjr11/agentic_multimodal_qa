"""
Step 3: Fine-tune MedGemma 4B IT on VQA-RAD
- Full bf16 intended for H100 / large-memory GPUs
- LoRA on attention + MLP layers
- Yes/no oversampled 3x
- Early stopping on validation loss

Notes:
- This is a starting point, not a guaranteed final recipe.
- If your Transformers version does not support MedGemma with
  AutoModelForImageTextToText, update the environment first.
"""
...
def main():
    ...
    def fast_collator(batch):
        ...
        if "token_type_ids" in batch[0]:
            ...
            token_type_ids_padded = torch.zeros(
                (bs, max_len), dtype=token_type_ids[0].dtype
            )
    ...
    yn_samples = [
        sample for sample in all_train
        if sample.get("answer", "").strip().lower() in ("yes", "no")
    ]
    ...
    # ========== TRAINING ==========
    print("\n" + "=" * 60)
    print("TRAINING: MedGemma 4B IT, LoRA r=16, lr=5e-5, 600 steps, yes/no 3x")
    print("=" * 60)
